...
void update_debug_title() {
  ...
  title_bar_canvas->print(String(M5Cardputer.Power.getBatteryLevel()) + "%");
...
void init_ui() {
  // 创建标题栏画布
  title_bar_canvas = new M5Canvas(&M5Cardputer.Display);
...
void handleInput() {
  ...
  bool curr_enter = M5Cardputer.Keyboard.isKeyPressed(KEY_ENTER);
  ...
  bool pressed_enter = curr_enter && !prev_enter;
